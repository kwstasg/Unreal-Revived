...
if (Get-Process -Name Unreal -ErrorAction SilentlyContinue) { throw 'Close the game before testing menu back navigation.' }
$systemDir = Join-Path $gameRoot 'System64'
$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$iniName = "MenuBack-$stamp.ini"
$userName = "MenuBackUser-$stamp.ini"
$logName = "MenuBack-$stamp.log"
...
try {
    $testProcess = Start-Process -FilePath (Join-Path $systemDir 'Unreal.exe') -WindowStyle Hidden -PassThru `
        -WorkingDirectory $systemDir -ArgumentList @('NyLeve?Game=ModernMenu.ModernMenuBackTestGame',
            "ini=$iniName", "userini=$userName", "log=$logName", '-novr', '-nosplash')
    if (!$testProcess.WaitForExit(30000)) { throw "Menu back navigation timed out: $logPath" }
    $log = Get-Content -LiteralPath $logPath -Raw
    if ($testProcess.ExitCode -ne 0 -or $log -notmatch 'MENUBACKTEST completed' -or
        $log -match 'ScriptWarning|Critical Error|Assertion failed|invalid outer|broken pointer|MENUBACKTEST FAIL') {
        throw "Menu back navigation failed: $logPath"
    }
    Write-Output "Menu back navigation and shutdown passed: $logPath"
}
finally {
